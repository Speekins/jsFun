...
const classPrompts = {
  ...
  totalCapacities() {
    ...
    return classrooms.reduce((capacities, classroom) => {
      if (classroom.program === 'FE') {
        capacities.feCapacity ? capacities.feCapacity += classroom.capacity : capacities.feCapacity = classroom.capacity;
      } else {
        capacities.beCapacity ? capacities.beCapacity += classroom.capacity : capacities.beCapacity = classroom.capacity;
      }
      return capacities;
    }, {})
...
const weatherPrompts = {
  getAverageTemps() {
    ...
    return weather.map(city => average(city.temperature.high, city.temperature.low));
  ...
  findSunnySpots() {
    ...
    return weather.reduce((array, currentCity) => {
      if (currentCity.type.includes('sunny')) {
        array.push(`${currentCity.location} is ${currentCity.type}.`);
      }
      return array;
    }, []);
...
const ultimaPrompts = {
  totalDamage() {
    ...
    return characters.reduce((damage, character) => {
      character.weapons.forEach(weapon => damage += weapons[weapon].damage)
      return damage
    }, 0)
    ...
    return characters.map(character => {
      ...
      for (let weapon in weapons) {
        if (character.weapons.includes(weapon)) {
          ...
          rangeTotal += weapons[weapon].range;
        }
      }
